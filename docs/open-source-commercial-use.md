...
前端侧栏提供“开源许可”入口，直接打开当前版本的 `/THIRD_PARTY_LICENSES.txt`；入口在桌面、折叠侧栏和 390px 窄屏均可操作，键盘 Tab 聚焦时显示高对比轮廓。开发环境由 Vite 在 SPA 回退前精确拦截该路径，复用有大小和供应链校验的许可证生成器在内存中生成正文；锁文件或生产目标变化会使缓存失效，生成失败只返回通用中文 503，不能伪装成首页 200。生产 Nginx 对该精确路径附加安全响应头、UTF-8 字符集并设置 `Cache-Control: no-store`，避免中文声明乱码或无内容哈希的许可证文件被缓存为旧版本。HTTP 与浏览器回归还会确认公开正文完整且不包含发布清单字段。
