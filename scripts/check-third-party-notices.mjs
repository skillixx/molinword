...
assert.equal(sanitizePublicSource("https://gitlab.com/example/glpat-12345678901234567890/artifact", "example", "1.0.0"), "https://www.npmjs.com/package/example/v/1.0.0", "GitLab 凭据不能进入公开产物");
assert.equal(sanitizePublicSource("https://github.com/example/xoxb-12345678901234567890/artifact", "example", "1.0.0"), "https://www.npmjs.com/package/example/v/1.0.0", "其他凭据前缀不能进入公开产物");
...
for (const entry of result.entries) {
  ...
  const expectedName = entry.name.split("/").map(encodeURIComponent).join("/");
  const expectedSource = `https://www.npmjs.com/package/${expectedName}/v/${encodeURIComponent(entry.version)}`;
  assert.equal(publicSource.toString(), expectedSource, `${marker} 只能公开由锁定包坐标构造的 npm 页面`);
}
